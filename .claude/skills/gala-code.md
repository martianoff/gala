...
6. **Expression-bodied functions** - Use `func f() T = expr` for single-expression functions
7. **Implicit typing** - Omit type params where inferable: `Some(42)` not `Some[int](42)`, `(x) => x * 2` not `(x int) => x * 2`
8. **Generics over reflection** - Use type parameters for reusable code
9. **Copy for updates** - Use `.Copy(field = newValue)` instead of mutation
10. **String interpolation** - Use `s"Hello $name"` instead of `fmt.Sprintf("Hello %s", name)`. Use `f"$x%.2f"` for explicit format control. No `import "fmt"` needed.
11. **Println/Print** - Use `Println(...)` and `Print(...)` instead of `fmt.Println(...)` / `fmt.Print(...)`. No import needed.
