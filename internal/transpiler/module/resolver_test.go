...
// TestResolveChildDeclaringModule covers Strategy 5.5: a search path that is a
// COLLECTION of modules rather than a module itself.
//
// That is the extracted stdlib cache's shape — ~/.gala/stdlib/<ver>/ has no
// module file, and each package beneath it carries a generated
// `module martianoff/gala/<pkg>`. Strategy 5 walks up from a search path and so
// never sees them, which left a fully-qualified stdlib path unresolvable from
// any project not itself named martianoff/gala.
func TestResolveChildDeclaringModule(t *testing.T) {
	root := t.TempDir()

	mk := func(dir, modFile, modulePath string) string {
		full := filepath.Join(root, dir)
		require.NoError(t, os.MkdirAll(full, 0755))
		require.NoError(t, os.WriteFile(filepath.Join(full, modFile),
			[]byte("module "+modulePath+"\n\ngo 1.22\n"), 0644))
		require.NoError(t, os.WriteFile(filepath.Join(full, "a.gala"),
			[]byte("package "+dir+"\n\nfunc F() int = 1\n"), 0644))
		return full
	}
	stdlibPkg := mk("test", "go.mod", "martianoff/gala/test")
	thirdParty := mk("widgets", "gala.mod", "example.com/toolkit/widgets")

	r := NewResolver([]string{root})

	t.Run("resolves a stdlib-shaped path", func(t *testing.T) {
		got, err := r.ResolvePackagePath("martianoff/gala/test")
		require.NoError(t, err)
		assert.Equal(t, stdlibPkg, got)
	})

	t.Run("resolves a third-party path by the same mechanism", func(t *testing.T) {
		// The standard library gets no special treatment: both resolve by
		// reading the module path the directory declares.
		got, err := NewResolver([]string{root}).ResolvePackagePath("example.com/toolkit/widgets")
		require.NoError(t, err)
		assert.Equal(t, thirdParty, got)
	})

	t.Run("a wrong prefix does not resolve on the trailing segment", func(t *testing.T) {
		_, err := NewResolver([]string{root}).ResolvePackagePath("example.com/wrong/test")
		assert.Error(t, err)
	})
}

// TestDeclaredModulePath pins the BOM guard. TrimSpace does not drop a U+FEFF,
// so without StripBOM a leading BOM hides the module directive and silently
// demotes the module to a plain directory — the bug findGalaModuleRoot already
// carries a comment about.
func TestDeclaredModulePath(t *testing.T) {
	t.Run("declares neither file", func(t *testing.T) {
		assert.Empty(t, DeclaredModulePath(t.TempDir()))
	})

	t.Run("go.mod", func(t *testing.T) {
		dir := t.TempDir()
		require.NoError(t, os.WriteFile(filepath.Join(dir, "go.mod"),
			[]byte("// Code generated by GALA transpiler. DO NOT EDIT.\nmodule martianoff/gala/test\n\ngo 1.22\n"), 0644))
		assert.Equal(t, "martianoff/gala/test", DeclaredModulePath(dir))
	})

	t.Run("gala.mod wins over go.mod", func(t *testing.T) {
		dir := t.TempDir()
		require.NoError(t, os.WriteFile(filepath.Join(dir, "go.mod"),
			[]byte("module example.com/go\n\ngo 1.22\n"), 0644))
		require.NoError(t, os.WriteFile(filepath.Join(dir, "gala.mod"),
			[]byte("module example.com/gala\n\ngala 0.80.0\n"), 0644))
		assert.Equal(t, "example.com/gala", DeclaredModulePath(dir))
	})

	t.Run("a leading BOM does not hide the directive", func(t *testing.T) {
		dir := t.TempDir()
		require.NoError(t, os.WriteFile(filepath.Join(dir, "gala.mod"),
			[]byte("\ufeffmodule example.com/bommed\n\ngala 0.80.0\n"), 0644))
