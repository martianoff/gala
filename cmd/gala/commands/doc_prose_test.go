...
// TestFindPackageDirByImportPath covers the fully-qualified spelling. A source
// file imports `martianoff/gala/test`, so that is the string a reader pastes;
// joining it onto a search path only ever matches the trailing segment, so the
// qualified form used to miss.
func TestFindPackageDirByImportPath(t *testing.T) {
	root := t.TempDir()

	// Two packages laid out the way the extracted stdlib and a vendored GALA
	// module both are: a directory per package, each declaring its own import
	// path.
	mk := func(dir, modulePath string) string {
		full := filepath.Join(root, dir)
		require.NoError(t, os.MkdirAll(full, 0755))
		require.NoError(t, os.WriteFile(filepath.Join(full, "go.mod"),
			[]byte("module "+modulePath+"\n\ngo 1.22\n"), 0644))
		require.NoError(t, os.WriteFile(filepath.Join(full, "a.gala"),
			[]byte("package "+dir+"\n\nfunc F() int = 1\n"), 0644))
		return full
	}
	testDir := mk("test", "martianoff/gala/test")
	otherDir := mk("widgets", "example.com/toolkit/widgets")

	paths := []string{root}

	t.Run("resolves a stdlib-shaped path", func(t *testing.T) {
		assert.Equal(t, testDir, findPackageDirByImportPath(paths, "martianoff/gala/test"))
	})

	t.Run("resolves a third-party path by the same mechanism", func(t *testing.T) {
		// The standard library gets no special treatment here: both resolve by
		// reading the module line the directory declares.
		assert.Equal(t, otherDir, findPackageDirByImportPath(paths, "example.com/toolkit/widgets"))
	})

	t.Run("a path no directory declares does not resolve", func(t *testing.T) {
		assert.Empty(t, findPackageDirByImportPath(paths, "martianoff/gala/nosuch"))
	})

	t.Run("a wrong prefix does not resolve on the trailing segment alone", func(t *testing.T) {
		assert.Empty(t, findPackageDirByImportPath(paths, "example.com/wrong/test"))
	})

	t.Run("suggests the bare name when a qualified path misses", func(t *testing.T) {
		assert.Equal(t, "test", docBareNameSuggestion(paths, "example.com/wrong/test"))
	})
}
...
func TestDeclaredModulePath(t *testing.T) {
	dir := t.TempDir()
	assert.Empty(t, declaredModulePath(dir), "a directory declaring neither file has no module path")
	...
	// gala.mod wins when both are present.
	require.NoError(t, os.WriteFile(filepath.Join(dir, "gala.mod"),
		[]byte("module example.com/pref\n\ngala 0.80.0\n"), 0644))
	assert.Equal(t, "example.com/pref", declaredModulePath(dir))
}
