...
Named arguments work with struct construction, `Copy()` method overrides, function calls and method calls — including methods with type parameters of their own, such as `arr.FoldLeft(initial = 0, f = (acc, x) => acc + x)`.
...
Collections are built through these named constructor functions — `ListOf`,
`ArrayOf`, `HashMapOf`, `EmptyList` — never by calling the type itself. Scala's
`List(1, 2, 3)` is [GALA-E0043](errors/GALA-E0043.md), and calling a method a
type does not declare is [GALA-E0044](errors/GALA-E0044.md).
...
A shorthand struct's field list reuses the parameter syntax, so a field can
declare a default. A construction that omits the field gets that default:

```gala
struct Cfg(Name string, Mask rune = '•', Tries int = 3)

val a = Cfg(Name = "a")            // Mask = '•', Tries = 3
val b = Cfg("b")                   // same, positionally
val c = Cfg(Name = "c", Tries = 9) // Mask = '•', Tries = 9
```

Defaults are evaluated at each construction, not once at declaration — the same
rule [default parameter values](#default-parameter-values) follow:

```gala
struct Entry(Msg string, At time.Time = time.Now())
```

Omitting a field that declares **no** default is an error
([GALA-E0045](errors/GALA-E0045.md)), so a constructor call cannot quietly leave
a zero where a value belongs:
...
The check applies to **shorthand-declared GALA structs only**. Everything that is
or mirrors a Go struct keeps Go's partial-literal semantics, because Go's
zero-value contract is what the Go boundary runs on:

| Form | Partial construction | Consults defaults |
|------|----------------------|-------------------|
| `struct Cfg(...)` via call syntax — `Cfg(Name = "a")`, `Cfg("a")` | rejected | yes |
| `struct Cfg(...)` via literal — `Cfg{Name: "a"}` | allowed | no |
| `type Cfg struct { ... }` — any syntax | allowed | n/a (cannot declare one) |
| A Go-imported type — `url.URL{...}` or `url.URL(Scheme = "x")` | allowed | n/a |

This is the Go-interop boundary, not an inconsistency:

- **A Go struct can only be constructed partially.** `url.URL` has ten fields and
  callers set two; `http.Server` and `tls.Config` are the same shape. Requiring
  every field would make the entire Go standard library unconstructable. So a
  Go-imported type stays partial *whichever syntax names it* — the call form is
  sugar for the literal, not a GALA constructor.
- **The block form is GALA's Go-shaped struct.** `type Cfg struct { ... }`
  mirrors a Go declaration field for field, which is what makes it the right
  choice for a type crossing into Go — and it has no syntax for a field default,
  so requiring every field would leave no way to opt out. The standard library
  depends on this: a HAMT node in `collection_immutable` sets two of its four
  fields and means the rest to be zero.
- **`Cfg{...}` is a composite literal, not a constructor call.** The braces are
  Go's spelling, and they behave like Go's: omitted fields take the zero value
  and no default is consulted. Reach for them when you want exactly that.
...
value. Prefer it for GALA-native types; use the block form for types that exist
to meet Go.

`Copy` is unaffected by all of this: it carries every field forward from the
receiver, so an omitted field keeps its current value rather than being
re-defaulted.
...
`type X Y` declares an **alias**: a second name for an existing type, not a new
type. The alias and its target are the same type and are interchangeable
everywhere — a value of one *is* a value of the other, with no conversion and no
separate identity.
...
Aliases transpile to Go type aliases (`type X = Y`), which is what makes them
transparent: methods on the target are available through the alias, and an
alias declared in one file is visible to every file in the same package.
...
**It cannot take a method unless its target is a plain type declared in this
package.** The receiver base type is the *target*, and Go accepts a method only
on a plain, locally declared type — so a built-in, an imported type, an unnamed
composite (slice, map, func) and an instantiated type are all
[GALA-E0048](errors/GALA-E0048.md):
...
The alias chain is followed to its end, so `type A int64; type B A` makes a
method on `B` illegal for the same reason. A pointer target is fine when it
points at a local type — `type PP *Point` puts the method on `Point`.
...
**It does not create a distinct type.** An alias cannot be matched, overloaded
or type-switched apart from its target, and it does not make an illegal
assignment illegal — `MyString` and `string` are one type. GALA has no newtype
declaration; a single-field struct is the way to get a separate identity.
...
### Lambda Expressions
Lambdas use the `=>` syntax. Parameter types can be omitted when inferrable from context (e.g., as arguments to typed methods). The parameter list is always parenthesized, including for a single parameter — a bare `x => e` is [GALA-E0042](errors/GALA-E0042.md).
...
**Compile-time data-race safety.** A `Future` body is a `Sendable[func() T]` *concurrency boundary*: the transpiler statically checks that the async closure only captures immutable, shareable values. Capturing a reassignable `var` or a `val` of a non-immutable type (e.g. a `collection_mutable` value, or a struct with a `var` field) is rejected with [GALA-E0037](errors/GALA-E0037.md). Snapshot such data into an immutable `val`, or use `collection_immutable`. `Sendable[F]` is a transparent, general marker — it is exactly `F` at runtime, and any library can annotate its own boundary parameters with it. `go_interop.Spawn` is the low-level, *unchecked* escape hatch. See [CONCURRENCY_SAFETY.MD](CONCURRENCY_SAFETY.MD).
...
## 11. Go Built-in Functions Are Forbidden

Bare Go builtins are **not** part of GALA's surface. Calling one is a hard
transpile error (**GALA-E0035**). They were the last symbols that resolved with
no import and no GALA declaration — a silent Go-leakage special case GALA
removes. Each has a GALA-native form or a sanctioned interop wrapper:
...
The check is **resolver-aware**: a user-defined function that happens to share a
builtin's name (e.g. `func delete(...)`) is that function, not the builtin, and
is left alone.

> `.Size()` fixes a real footgun: Go's `len(string)` returns **bytes**, so any
> non-ASCII text mis-counts. `.Size()` means logical size (`"héllo".Size() == 5`);
> `.ByteSize()` is there when you actually want bytes (`== 6`).
...
For guaranteed resource cleanup, prefer the `resource` combinators
...
### The `if` initializer statement is forbidden

Go's `if init; cond { … }` — a binding, a `;`, then the condition — is **not**
part of GALA's statement surface. A use is a hard transpile error
(**GALA-E0047**).
...
A Go `(T, error)` call is the shape this form is reached for, and `Try` wraps
exactly that shape — the pair is auto-wrapped into `Success` / `Failure`:
...
// Or, when the failure is simply a fallback:
func parseOr2(text string, fallback int) int =
    Try(strconv.Atoi(text)).GetOrElse(fallback)
```

Only the initializer slot is rejected. A plain condition, `else`, `else if`,
and the if-*expression* `if (cond) a else b` are all unaffected.
...
```gala
func doPanic() { go_builtins.Panic("boom") }
...
- [Subprocess](SUBPROCESS.MD) - Spawn and drive external child processes with a goroutine-safe `Process` handle.
...
- **Go to Definition** — cross-file navigation via analyzer metadata (a directory holding a single `main` program is analyzed as one package), local declarations, pattern bindings, named arg fields
- **Find References** — occurrences across all files of the package
- **Completion** — type-aware dot completion (fields, methods), sealed case patterns with destructuring, keywords, built-in functions; documentation is fetched per entry as it is highlighted
  - Completing a method or function writes out its required parameters as named arguments, with a tab stop for each value: `WithName(name = |)`
  - Inside an argument list, the parameters not given yet are offered as named arguments first — for functions, methods (resolved through builder chains) and constructors
...
- **Document symbols** — the file's own types, sealed variants, methods, functions and package-level vals in the document outline
- **Workspace symbols** — declarations matching a name across every `.gala` file under the project root (hidden directories and Bazel output skipped)
...
Install the GALA extension from the [releases page](https://github.com/martianoff/gala/releases) — `code --install-extension gala-vscode-<version>.vsix`, or **Extensions** > `...` > **Install from VSIX...**. Unlike VS Code's built-in language support, the extension registers the `gala` language for `.gala` files and starts `gala lsp`; it requires the GALA CLI on `PATH`, or `gala.serverPath` / `GALA_PATH` pointing at it.

This gives you syntax highlighting plus diagnostics, hover, go-to-definition, references, completion, signature help, and inlay hints — all the LSP features listed above.
